...
def load_model_and_alphabet(model_location):
    #check file directory instead here
    if not model_location.is_file():
        print("ESM-1b model not found, downloading to local.")
        return load_model_and_alphabet_hub("esm1b_t33_650M_UR50S", model_location.parent)
    else:
        return load_model_and_alphabet_local(model_location)

#    if model_name.name.endswith(".pt"):  # treat as filepath
#        return load_model_and_alphabet_local(model_name)
#    else:
#        return load_model_and_alphabet_hub(model_name)

def load_hub_workaround(url, model_save_path):
    try:
        data = torch.hub.load_state_dict_from_url(url, model_dir=model_save_path, map_location="cpu")
    except RuntimeError:
...
def load_regression_hub(model_name, model_save_path):
    url = f"https://dl.fbaipublicfiles.com/fair-esm/regression/{model_name}-contact-regression.pt"
    regression_data = load_hub_workaround(url, model_save_path)
    return regression_data

def load_model_and_alphabet_hub(model_name, model_save_path):
    url = f"https://dl.fbaipublicfiles.com/fair-esm/models/{model_name}.pt"
    model_data = load_hub_workaround(url, model_save_path)
    if _has_regression_weights(model_name):
        regression_data = load_regression_hub(model_name, model_save_path)
    else:
